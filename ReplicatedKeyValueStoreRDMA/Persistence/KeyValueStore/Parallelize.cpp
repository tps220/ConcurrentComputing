...
Parallelize::Parallelize(Cuckoo<int, int>* set, unsigned int range, unsigned int nb_threads) : 
...
void execute_local_txs(Thread_Data *thread, Cuckoo<int, int>* set) {
  int unext, last = -1; 
  val_t val = 0, val_2 = 0;
  ...
  while (stop == 0) {
    if (unext) { //write tx
      ...
      if (insert) { //add        
        ...
        RESULT result = set -> insert(element);
        if (result == RESULT::TRUE) {
          data -> nb_added++;
          last = val;
        }
        else if (result == RESULT::ABORT_FAILURE) {
          data -> nb_abort++;
        }
        data -> nb_add++;
      } else { // remove  
        if (data ->alternate) { // alternate mode  
          RESULT result = set -> remove(last);
          if (result == RESULT::TRUE) {
            data -> nb_removed++;
          }
          last = -1;  
